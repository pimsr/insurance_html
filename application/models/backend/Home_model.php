...
class Home_model extends CI_Model{
	...
	public function insertReview($value){
		...
		$this->db->insert('tb_review',$values);
		return $this->db->affected_rows();
	}
	public function updateReview($value){
		$values = array(
			'title_th' => $value['title_th'],
			'title_en' => $value['title_en'],
			'subtitle_th' => $value['subtitle_th'],
		...
		$this->db->where('id', $value['id']);
		$this->db->update('tb_review', $values);
		return $this->db->affected_rows();
	}
	public function updateReviewActions($data){
		foreach($data['id'] as $key => $id){
			$values = array(
				'seq' => $data['seq'][$key],
				'status' => empty($data['status'][$key]) ? 0 : 1
	...
	public function deleteReview($id){
		$this->db->where('id', $id);
		$this->db->update('tb_review', array('status' => -1));
		return $this->db->affected_rows();
	}
